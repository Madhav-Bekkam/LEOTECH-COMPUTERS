  ...
  return (
    <>
      ...
      {landingData.visibility?.stats !== false && (
        <motion.div initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.3, margin: "100px" }} transition={{ duration: 0.6 }} className="max-w-7xl mx-auto w-full px-4 md:px-6 mb-20 md:mb-32 relative z-10">
      ...
      {landingData.visibility?.whyUs !== false && (
        <section className="py-16 md:py-24 px-4 md:px-6 relative z-10 bg-gradient-to-b from-transparent via-[#0A0F1E]/80 to-transparent backdrop-blur-sm border-y border-white/5">
          <div className="max-w-7xl mx-auto">
            <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "100px" }} transition={{ duration: 0.6 }} className="text-center mb-12 md:mb-20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-10 perspective-[1000px]">
              <motion.div initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "100px" }} transition={{ duration: 0.5, delay: 0.1 }}>
              ...
              <motion.div initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "100px" }} transition={{ duration: 0.5, delay: 0.3 }}>
              ...
              <motion.div initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "100px" }} transition={{ duration: 0.5, delay: 0.5 }}>
      ...
      {landingData.visibility?.featured !== false && (
        <section className="py-16 md:py-24 px-4 md:px-6 relative z-10">
          <div className="max-w-7xl mx-auto perspective-[2000px]">
            <motion.div initial={{ opacity: 0, x: -50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, margin: "100px" }} transition={{ duration: 0.6 }} className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 md:mb-16 gap-6">
              <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
              {landingData.featuredCourses.map((fc, i) => {
                ...
                return (
                  <motion.div key={i} initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.1, margin: "100px" }} transition={{ duration: 0.5, delay: i * 0.15 }} className="h-full">
      ...
      {landingData.visibility?.techStack !== false && (
        <section className="py-24 px-6 relative z-10 bg-black/60 border-t border-white/5 overflow-hidden backdrop-blur-sm">
          <div className="max-w-7xl mx-auto text-center relative perspective-[1000px]">
            <motion.p initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "100px" }} transition={{ duration: 0.6 }} className="text-sm font-bold text-slate-400 uppercase tracking-widest mb-12 drop-shadow-lg">Industry Tools You Will Master</motion.p>
            <div className="flex flex-wrap justify-center gap-6 md:gap-10 opacity-90 transition-opacity duration-500">
              {landingData.techStack.filter(t => t.trim() !== '').map((tech, i) => (
                <motion.div key={i} initial={{ opacity: 0, scale: 0.5, y: 30 }} whileInView={{ opacity: 1, scale: 1, y: 0 }} viewport={{ once: true, margin: "100px" }} transition={{ duration: 0.5, type: 'spring', bounce: 0.4, delay: i * 0.05 }} style={{ transform: `translateZ(${Math.random() * 30 + 10}px)` }}>
                  <TiltCard 
                    onClick={() => {
                      if (isAdmin) return;
                      if (!isAuthenticated) {
